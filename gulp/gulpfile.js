...
var config = {
  ...
  assets_in: package.config.assets_src + "/assets/**",
  assets_out: package.config.assets_dist + "/assets/",
...
gulp.task("css-vendor-maps", function() {
  return gulp
    .src(package.config.css_deps_maps)
    .pipe(gulpif(package.config.debug, debug()))
    .pipe(gulp.dest(config.css_out))
    .pipe(gulpif(package.config.debug, debug()));
});
...
gulp.task("js", function() {
  return (
    gulp
      ...
      // .pipe(gulpif(package.config.debug, uglify()))
      .pipe(header(banner, { package: package }))
      .pipe(rename({ suffix: ".min" }))
...
gulp.task("js-vendor-header", function() {
  return gulp
    .src(package.config.js_deps_header)
    .pipe(gulpif(package.config.debug, debug()))
...
gulp.task("js-vendor-header-maps", function() {
  return gulp
    .src(package.config.js_deps_header_maps)
    .pipe(gulpif(package.config.debug, debug()))
    .pipe(gulp.dest(config.js_vendor_out))
    .pipe(gulpif(package.config.debug, debug()));
});
...
gulp.task("js-vendor-footer", function() {
  return gulp
    .src(package.config.js_deps_footer)
    .pipe(gulpif(package.config.debug, debug()))
...
gulp.task("js-vendor-footer-maps", function() {
  return gulp
    .src(package.config.js_deps_footer_maps)
    .pipe(gulpif(package.config.debug, debug()))
    .pipe(gulp.dest(config.js_vendor_out))
    .pipe(gulpif(package.config.debug, debug()));
});
...
gulp.task("js-opt", function() {
  return gulp
    .src(package.config.js_deps_opt)
    .pipe(gulpif(package.config.debug, debug()))
...
gulp.task("js-opt-maps", function() {
  return gulp
    .src(package.config.js_deps_opt_maps)
    .pipe(gulpif(package.config.debug, debug()))
    .pipe(gulp.dest(config.js_opt_out))
    .pipe(gulpif(package.config.debug, debug()));
});
...
gulp.task("assets", function() {
  return gulp
    .src(config.assets_in)
    .pipe(gulpif(package.config.debug, debug()))
    .pipe(gulp.dest(config.assets_out))
    .pipe(gulpif(package.config.debug, debug()));
});
...
gulp.task("write_config", function() {
  return fs.writeFileSync(
    package.config.assets_dist + "/config.js",
    JSON.stringify(package.config)
  );
});

gulp.task("revision", [], () => {
  return gulp
    .src("assets/dist/!**!/!*.{css,js,jpg,jpeg,png,gif,icon}")
    .pipe(gulpif(package.config.debug, debug()))
    .pipe(rev())
    .pipe(gulpif(package.config.debug, debug()))
...
gulp.task("revRewrite", ["revision"], function() {
  const manifest = gulp.src("assets/dist/rev-manifest.json");
